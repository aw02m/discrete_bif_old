...
Eigen::VectorXd func_newton(Eigen::VectorXd v, dynamical_system* ds)
{
    Eigen::VectorXd x = v(Eigen::seqN(0, ds->xdim));
    ds->params[ds->var_param] = v(ds->xdim);
    Eigen::VectorXd ret(ds->xdim+1);
    Eigen::MatrixXd I = Eigen::MatrixXd::Identity(ds->xdim, ds->xdim);
    double mu = -1;

    ret(Eigen::seqN(0, ds->xdim)) = Tl(x, ds) - x;
    ret(ds->xdim) = (dTldx(x, ds) - mu*I).determinant();

    return ret;
}

Eigen::MatrixXd jac_newton(Eigen::VectorXd v, dynamical_system* ds)
{
    Eigen::VectorXd x = v(Eigen::seqN(0, ds->xdim));
    ds->params[ds->var_param] = v(ds->xdim);
